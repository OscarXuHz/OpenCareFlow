...
SOURCE_PDF_DIR = settings.data_path / "templates"


def _with_source_status(template: dict[str, Any]) -> dict[str, Any]:
    """Annotate a mapping without treating third-party PDFs as bundled code."""
    template = dict(template)
    source_pdf = template.get("source_pdf")
    source_available = bool(
        isinstance(source_pdf, str)
        and source_pdf
        and (SOURCE_PDF_DIR / source_pdf).is_file()
    )
    template["source_available"] = source_available
    if not source_available and template.get("status", "ready") == "ready":
        template["status"] = "source_required"
    return template
...
def list_templates() -> list[dict[str, Any]]:
    ...
    for fp in sorted(TEMPLATE_DIR.glob("*.json")):
        if fp.name.startswith("_"):
            continue
        try:
            t = _with_source_status(json.loads(fp.read_text(encoding="utf-8")))
        ...
        out.append({
            ...
            "source_available": t.get("source_available", False),
...
def load_template(template_id: str) -> dict[str, Any]:
    ...
    return _with_source_status(json.loads(fp.read_text(encoding="utf-8")))
