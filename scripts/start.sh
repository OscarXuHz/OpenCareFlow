...
# FastAPI has no application authentication. Keep direct development access
# on loopback; shared deployments must use the authenticated Nginx boundary.
nohup uvicorn app.main:app --host 127.0.0.1 --port 8000 > "$ROOT/backend.log" 2>&1 &
BACKEND_PID=$!
