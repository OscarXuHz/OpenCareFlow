...
When enabled, these channels transmit workflow content outside the CareFlow host: text/transcripts go to the configured text endpoint, images or rendered PDF pages go to Azure, and audio is uploaded through DashScope's temporary OSS/ASR flow. Provider region, retention, logging, training use, and deletion are deployment decisions rather than guarantees made by this codebase.
...
The Fernet key and ciphertext currently live under the same data root. This protects against casual plaintext inspection, not compromise of the whole volume or its backups. The burn operation does not delete the original audio, database metadata, generated output, backups, downloads, or provider-side copies.

Raw vision-provider responses are not persisted by default. Setting `STORE_AI_RAW_RESPONSES=true` opts into duplicating provider output in SQLite for short-lived debugging and requires a separate retention plan.

Third-party welfare-form PDFs are not part of the public source tree. Version-specific JSON mappings remain, and operators must obtain permitted source PDFs separately.
...
This is a limited demo guardrail, not a production authentication model. FastAPI itself has no authentication; exposing its port directly bypasses Nginx Basic Auth. Shared access requires an authenticated proxy or application-level identity, plus role-based permissions and audit logging.
